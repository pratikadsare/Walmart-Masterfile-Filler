...
# >>> IMPORTANT: Walmart template specifics
# We map using titles in ROW 6 and write data starting on ROW 7.
HEADER_ROW_INDEX = 6
DATA_START_ROW = 7
...
def _extract_headers_row(ws, header_row: int) -> Tuple[List[str], Dict[str, int]]:
    """Read the header row (row 6) into a list and a normalized->column index map (1-based)."""
    headers: List[str] = []
...
def _import_mapping(df: pd.DataFrame, apply_sheet_key: Optional[str],
    ...
    tcol = None
    for c in ["template", "templateheader", "templ", "target", "targetheader"]:
        if c in cols:
            tcol = cols[c]; break
    rcol = None
    for c in ["raw", "rawheader", "source", "sourceheader"]:
        if c in cols:
            rcol = cols[c]; break
    scol = None
    for c in ["sheet", "tab", "worksheet"]:
        if c in cols:
            scol = cols[c]; break
...
def _write_sheet_data(ws, mapping: List[Dict[str, str]],
    ...
    # Map normalized header -> column index from the sheet's header row (row 6)
...
with tab1:
    ...
    if tpl is not None:
        ...
        # Load workbook and discover target sheets and their ROW 6 headers
        ...
        # Extract headers for each present target sheet from ROW 6
        templ_headers_by_sheet: Dict[str, List[str]] = {}
        ...
        # If headers changed for a sheet, clear only that sheet's mapping keys
...
# =============================
# Notes
# - Headers read from ROW 6; data written from ROW 7.
